...
def mutate(arr, mu=0.01):
    ...
    for i in range(0, len(bits)):
        if random() < mu:
            bits[
                i
            ] ^= 1  # We make use of the array consisting of only bits and do an XOR to flip the bit
    return bits
...
def question1(bit_string, generations=1500, mu=0.01):
    ...
    """
    best_fitness = np.zeros(
        generations
    )  # keep track of the best found fitness per generation
    ...
    for i in range(0, generations):
        ...
        ):  # conditionally update best_fitness if we find a new optimum
        ...
        if (
            calculate_fitness(xm) > fitness
        ):  # conditionally update bit_string if we have found a better string
            bit_string = xm
    return best_fitness
...
def question4(generation=1500, mu=0.01, nr_trials=1000):
    ...
    for i in range(0, nr_trials):
        avg_fitness1 = avg_fitness1 + question1(
            initialise_bitstring(l), generations, mu
        )
        avg_fitness2 = avg_fitness2 + question2(
            initialise_bitstring(l), generations, mu
        )
